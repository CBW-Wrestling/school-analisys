...
function getContactLabel(mode: Shipment['mode']) {
  if (mode === 'land') {
    return 'Ligar para o motorista'
  }

  if (mode === 'air') {
    return 'Ligar para o suporte da companhia aérea'
  }

  return 'Ligar para o capitão'
}
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="icon-sm" aria-label="Copiar ID da remessa">
            <Copy />
          </Button>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs sm:text-sm">
          ...
          <span className="text-foreground tabular-nums">
            Previsão: {shipment.eta} {shipment.etaMeta}
          </span>
        </div>
      </div>
