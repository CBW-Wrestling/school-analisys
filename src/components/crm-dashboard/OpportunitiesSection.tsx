    ...
    <section>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Exemplo de acompanhamento de leads qualificados em fase de descoberta, proposta e fechamento.
          </CardDescription>
          <CardAction>
                  ...
                  >
                    {stageOptions.map((option) => (
                      <DropdownMenuRadioItem key={option} value={option}>
                        {option === 'all' ? 'Todos os estágios' : option}
                      </DropdownMenuRadioItem>
                    ))}
                  </DropdownMenuRadioGroup>
                </DropdownMenuContent>
                ...
                <DropdownMenuContent align="end" className="w-40">
                  ...
                  >
                    {healthOptions.map((option) => (
                      <DropdownMenuRadioItem key={option} value={option}>
                        {option === 'all' ? 'Todas as saúdes' : option}
                      </DropdownMenuRadioItem>
                    ))}
                  </DropdownMenuRadioGroup>
                </DropdownMenuContent>
        ...
        <CardContent className="flex flex-col gap-4 px-0">
          <div className="overflow-hidden">
            <Table className="**:data-[slot='table-cell']:px-4 **:data-[slot='table-head']:px-4 **:data-[slot='table-cell']:py-4">
              <TableHeader className="border-t **:data-[slot='table-head']:h-11 **:data-[slot='table-head']:font-medium **:data-[slot='table-head']:text-sm **:data-[slot='table-head']:text-foreground">
          ...
          <div className="flex items-center justify-between gap-4 px-4 pb-1">
            <p className="text-sm text-muted-foreground">
              Exibindo {visibleOpportunityCount} de {filteredOpportunityCount.toLocaleString('pt-BR')} oportunidades
            </p>
