...
// Série sintética (mesma forma/magnitude do bloco de referência analytics) — sem dado real de negócio.
const realtimeData = [
  { minute: 1, visitors: 0 },
  { minute: 2, visitors: 6 },
...
const topCountries = [
  { name: 'Estados Unidos', flag: '🇺🇸', visitors: 14 },
  { name: 'Reino Unido', flag: '🇬🇧', visitors: 4 },
  { name: 'Canadá', flag: '🇨🇦', visitors: 3 },
  { name: 'Índia', flag: '🇮🇳', visitors: 3 },
]
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex items-end justify-between">
          <div className="flex items-baseline gap-1">
            <span className="text-2xl leading-none tracking-tight tabular-nums">24</span>
        ...
        <div className="grid grid-cols-2">
          {topCountries.map((country, index) => (
            <div
              key={country.name}
              className={[
                'flex items-center gap-3 border-border/50 pt-1 pb-4',
                index % 2 === 0 ? 'border-r pr-5' : 'pl-5',
                index < 2 ? 'border-b pt-1' : 'pt-4',
              ].join(' ')}
            >
