...
import { ALL_FILTER, filters, type UserRow } from './data'
...
export function Users({ users }: { users: UserRow[] }) {
  ...
  function setColumnSelectFilter(columnId: string, value: string) {
    table.getColumn(columnId)?.setFilterValue(value === ALL_FILTER ? undefined : value)
    ...
    <Card>
      <CardHeader className="border-b has-data-[slot=card-action]:grid-cols-1 md:has-data-[slot=card-action]:grid-cols-[1fr_auto]">
        ...
        <CardDescription className="max-w-sm leading-snug">
          Gerencie os membros da organização e seus acessos.
        </CardDescription>
      ...
      <CardContent className="flex flex-col gap-4 px-0">
        ...
        <div className="flex items-center justify-between gap-3 px-4">
          <div className="text-sm text-muted-foreground tabular-nums">{selectedCount} selecionado(s)</div>
